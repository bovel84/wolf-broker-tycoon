...
(function (root, factory) {
	...
	var CHAPTERS = [
		{
			...
			triggerMissions: 0,
			intro:
				"Sei un giovane trader che entra nella Borsa. Hai €10.000 e un sogno.",
	...
	var MISSION_TEMPLATES = {
		...
		first_profit: {
			name: "Primo Profitto",
			desc: "Realizza un profitto positivo",
			xp: 75,
			cash: 1000,
			check: function (s) {
				return s.portfolio.realizedPnL > 0;
			},
			deadline: 0,
		},
		reach_25k: {
			name: "€25K di Patrimonio",
			desc: "Raggiungi €25.000 di patrimonio",
			xp: 75,
			cash: 2500,
			check: function (s) {
				return s.player.netWorth >= 25000;
			},
			deadline: 0,
		},
		make_50k: {
	...
	function rngGauss(mean, stdev) {
		...
		var u2 = Math.random();
		return (
			mean + stdev * Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2)
		);
	}
	function uid() {
	...
	GameEngine.prototype._generateCompanies = function (n) {
		...
		if (MarketEngineCtor) {
			...
			for (var i = 0; i < source.length; i++) {
				...
				var revenue =
					c.fundamentals && c.fundamentals.revenue ? c.fundamentals.revenue : 0;
				var profit =
					c.fundamentals && c.fundamentals.profit ? c.fundamentals.profit : 0;
				companies.push({
					id: c.id,
					ticker: c.ticker,
					name: c.name,
		...
		for (var i = 0; i < n; i++) {
			...
			companies.push({
				...
				prevClose: Math.max(0.1, price),
				open: Math.max(0.1, price),
				high: Math.max(0.1, price),
				low: Math.max(0.1, price),
				volume: rngInt(100000, 10000000),
				avgVolume: rngInt(100000, 10000000),
				marketCap: Math.max(0.1, price) * shares,
	...
	GameEngine.prototype._checkLevelUp = function () {
		...
		for (var i = LEVELS.length - 1; i >= 0; i--) {
			...
			if (
				p.xp >= lv.minXP &&
				p.netWorth >= lv.minNetWorth &&
				this._completedMissionsCount() >= lv.minMissions
			) {
				if (p.level < lv.id) {
					p.level = lv.id;
					leveledUp = true;
	...
	GameEngine.prototype.hasUnlock = function (unlockName) {
		...
		return (
			lv.unlocks.indexOf(unlockName) !== -1 || lv.unlocks.indexOf("all") !== -1
		);
	};
	...
	GameEngine.prototype._checkMissions = function () {
		...
		for (var i = missions.length - 1; i >= 0; i--) {
			...
			if (m.check && m.check(this.state)) {
				...
				m.deadline > 0 &&
				this.state.player.week - m.startWeek > m.deadline
			) {
				m.status = "failed";
	...
	GameEngine.prototype._checkAchievements = function () {
		...
		var checks = {
			first_trade: function () {
				return stats.totalTrades >= 1;
			},
			first_profit: function () {
			...
			fifty_trades: function () {
				return stats.totalTrades >= 50;
			},
			hundred_trades: function () {
				return stats.totalTrades >= 100;
			},
			five_hundred_trades: function () {
			...
			ghost_trader: function () {
				return (
					stats.insiderTrades >= 10 && self.state.player.reputation.sec > 30
				);
			},
			front_runner: function () {
				return stats.frontRunningExecuted >= 1;
			},
			bribe_master: function () {
				return stats.bribesPaid >= 5;
			},
			philanthropist: function () {
	...
	GameEngine.prototype._checkSECRaid = function () {
		...
		var darkCount =
			this.state.darkActions.pumpDumps.length +
		...
		var raidChance = 0;
		if (secRep < 80)
			raidChance += ((80 - secRep) / 200) * diffCfg.secAggression;
	...
	GameEngine.prototype._triggerSECRaid = function () {
		...
		var frozenAmount = Math.min(
			p.cash * 0.5,
			darkCount * 50000 * diffCfg.secAggression,
		);
		p.cash -= frozenAmount;
		...
		if (p.cash >= bribeCost && rng() < 0.4) {
			...
			if (rng() < 0.6) {
				...
				this.notify(
					...
					"Tangente Accettata",
					"Hai pagato " +
						formatMoney(bribeCost) +
						" per far sparire il problema.",
				);
				...
				this.notify(
					"danger",
					"Tangente Rifiutata!",
					"Il funzionario ha rifiutato e ha segnalato il tentativo.",
				);
	...
	GameEngine.prototype._spawnClient = function () {
		...
		if (this.state.clients.length > p.stats.maxClients)
			p.stats.maxClients = this.state.clients.length;
		...
		if (this.state.narrative) {
			this.addNarrativeMemory(
				"client:" + name.toLowerCase().replace(/[^a-z0-9]/g, "_"),
				name +
					" diventa cliente del broker affidando " +
					formatMoney(capital) +
					".",
				"positive",
				5,
	...
	GameEngine.prototype._processClients = function () {
		...
		for (var i = this.state.clients.length - 1; i >= 0; i--) {
			...
			if (this.state.narrative) {
				if (growth < -0.05) {
					this.addNarrativeMemory(
						"client:" + c.name.toLowerCase().replace(/[^a-z0-9]/g, "_"),
						c.name +
							" e' preoccupato per una perdita del " +
					...
					this.addNarrativeMemory(
						"client:" + c.name.toLowerCase().replace(/[^a-z0-9]/g, "_"),
						c.name +
							" e' entusiasta di un guadagno del " +
							(growth * 100).toFixed(1) +
							"% sul capitale.",
						"positive",
						5,
			...
			if (c.satisfaction < 20 && rng() < 0.3) {
				...
				this.notify(
					"client",
					"Cliente Perso",
					c.name + " ha ritirato il capitale.",
				);
	...
	GameEngine.prototype.hireAgent = function (name, skill, salary) {
		if (!this.hasUnlock("margin2x")) {
			this.notify(
				...
				"Devi essere livello 4+ per assumere agenti.",
			);
			return null;
		}
		var p = this.state.player;
		...
		if (p.cash < salary * 4) {
			this.notify(
				"warning",
				"Fondi insufficienti",
				"Servono almeno " + formatMoney(salary * 4) + " per assumere.",
			);
			return null;
		}
		var agent = {
		...
		this.notify(
			"success",
			"Agente Assunto!",
			name + " (" + skill + ") - Stipendio: " + formatMoney(salary) + "/sett",
		);
		this._checkAchievements();
		return agent;
	};
		...
		for (var i = this.state.agents.length - 1; i >= 0; i--) {
			...
			if (p.cash < 0) {
				...
				this.notify(
					"warning",
					"Agente Licenziato",
					a.name + " non pu\u00f2 essere pagato. Se n'\u00e8 andato.",
				);
			...
			if (a.loyalty < 20 && rng() < 0.05) {
				...
				this.notify(
					"danger",
					"Tradimento!",
					a.name + " ti ha tradito e rubato " + formatMoney(stolen) + "!",
				);
				this.emit("agentBetrayal", { agent: a, stolen: stolen });
				this.state.agents.splice(i, 1);
				if (this.state.narrative) {
					this.addNarrativeMemory(
						"agent:" + a.name.toLowerCase().replace(/[^a-z0-9]/g, "_"),
						a.name +
							" ha tradito il broker rubando " +
							formatMoney(stolen) +
							".",
						"negative",
						9,
			...
			if (this.state.narrative) {
				if (a.totalEarned > a.salary * 20) {
					this.addNarrativeMemory(
						"agent:" + a.name.toLowerCase().replace(/[^a-z0-9]/g, "_"),
						a.name + " e' salito di livello ed e' diventato piu' efficiente.",
						"positive",
						5,
					);
				}
			...
			if (rng() < 0.02) {
				...
				if (loss > 0) {
					...
					this.notify(
						"warning",
						"Errore Agente",
						a.name + " ha fatto un errore: -" + formatMoney(loss),
					);
				}
			}
		}
	...
	GameEngine.prototype._onChapterProgress = function () {
		...
		for (var i = CHAPTERS.length - 1; i >= 0; i--) {
			var ch = CHAPTERS[i];
			if (
				p.level >= ch.triggerLevel &&
				p.netWorth >= ch.triggerNetWorth &&
				this._completedMissionsCount() >= ch.triggerMissions
			) {
				if (p.chapter < ch.id) {
					p.chapter = ch.id;
	...
	GameEngine.prototype._getMoralChoices = function (chapterId) {
		var choices = {
			...
			10: [
				{
					id: "legacy_good",
					text: "Lascia un'eredit\u00e0 positiva",
					ethics: +10,
				},
				{
					id: "legacy_bad",
					text: "Lascia un'eredit\u00e0 oscura",
					ethics: -10,
				},
			],
		};
		...
		if (reason === "prison") {
			...
			if (p.stats.bankruptcies >= 2) {
				...
				this.notify(
					"danger",
					"Bancarotta!",
					"Hai perso tutto. Riparti da €10.000.",
				);
				this.emit("bankrupt", {});
				this._checkAchievements();
				return null;
	...
	GameEngine.prototype._serializeState = function () {
		...
		return {
			...
			meta: {
				...
				playtime:
					s.meta.playtime +
					(Date.now() - (s.meta.lastLoad || s.meta.createdAt)),
			},
		};
	};
	...
	GameEngine.prototype._deserializeState = function (data) {
		...
		if (!s.competitors) s.competitors = null;
		...
		if (!s.settings.difficultyConfig)
			s.settings.difficultyConfig =
				DIFFICULTY_CONFIG[s.settings.difficulty] || DIFFICULTY_CONFIG.normal;
		if (!s.meta)
			s.meta = {
				saveVersion: SAVE_VERSION,
				createdAt: Date.now(),
	...
	GameEngine.prototype.getCompany = function (tickerOrId) {
		...
		for (var i = 0; i < companies.length; i++) {
			if (companies[i].ticker === tickerOrId || companies[i].id === tickerOrId)
				return companies[i];
		}
		return null;
	};
	...
	GameEngine.prototype.buy = function (ticker, shares, options) {
		...
		var slippage =
			total * diffCfg.slippageRate * Math.min(1, shares / company.float);
		var cost = total + commission + slippage;

		// Level checks
		if (company.isPenny && !this.hasUnlock("penny"))
			return { success: false, error: "Livello troppo basso per penny stocks" };
		if (options.margin && !this.hasUnlock("margin" + options.margin + "x"))
			return { success: false, error: "Margin non sbloccato" };
		if (options.limit && !this.hasUnlock("limit"))
			return { success: false, error: "Limit orders non sbloccati" };
		if (options.short && !this.hasUnlock("short"))
			return { success: false, error: "Short selling non sbloccato" };
		...
		if (!options.short) {
			...
			if (pos) {
				var newAvg =
					(pos.avgPrice * pos.shares + price * shares) / (pos.shares + shares);
				pos.avgPrice = newAvg;
				pos.shares += shares;
				pos.totalCost += cost;
			} else {
			...
			if (shortPos) {
				...
				shortPos.avgPrice =
					(shortPos.avgPrice * shortPos.shares + price * shares) /
					(shortPos.shares + shares);
			} else {
	...
	GameEngine.prototype.sell = function (ticker, shares, options) {
		...
		if (pos && pos.shares >= shares) {
			...
			var slippage =
				proceeds * diffCfg.slippageRate * Math.min(1, shares / company.float);
			...
			var slip =
				coverCost * diffCfg.slippageRate * Math.min(1, shares / company.float);
	...
	GameEngine.prototype._recordTrade = function (
		type,
		company,
		shares,
		price,
		...
		if (this.state.portfolio.history.length > 500)
			this.state.portfolio.history.shift();
	};

	GameEngine.prototype._updateReputation = function (key, delta) {
		this.state.player.reputation[key] = clamp(
			this.state.player.reputation[key] + delta,
			0,
			100,
		);
	};
	...
	GameEngine.prototype.pumpAndDump = function (ticker) {
		...
		var p = this.state.player;
		var pos = this.state.portfolio.positions[company.id];
		if (!pos || pos.shares < 100)
			return { success: false, error: "Devi avere almeno 100 azioni" };
	...
	GameEngine.prototype.insiderTrade = function (ticker, action) {
		...
		var caught =
			rng() < 0.15 * this.state.settings.difficultyConfig.secAggression &&
			p.reputation.sec < 50;
	...
	GameEngine.prototype.frontRun = function (ticker, shares) {
		...
		var p = this.state.player;
		if (!this.hasUnlock("margin2x"))
			return { success: false, error: "Livello troppo basso" };
	...
	GameEngine.prototype.donate = function (amount) {
		...
		this.notify(
			"success",
			"Donazione",
			"Hai donato " + formatMoney(amount) + " in beneficenza. Etica +",
		);
		this._checkAchievements();
	...
	var ASSEMBLY_PROPOSALS = [
		{
			...
			name: "Aumento Dividendo",
			desc: "Aumentare il dividendo annuo del 25%",
			effect: "div+25",
			voteNeed: 0.5,
			impact: { type: "div", factor: 1.25, price: 1.03 },
		},
		{
			id: "dividend_cut",
			name: "Taglio Dividendo",
			desc: "Ridurre il dividendo per reinvestire",
			effect: "div-40",
			voteNeed: 0.5,
			impact: { type: "div", factor: 0.6, price: 0.98 },
		},
		{
			id: "buyback",
			name: "Buyback Azioni",
			desc: "Riacquistare azioni proprie sul mercato",
			effect: "buyback",
			voteNeed: 0.5,
			impact: { type: "buyback", price: 1.06 },
		},
		{
			id: "expansion",
			name: "Espansione Strategica",
			desc: "Aprire nuovi mercati e linee di prodotto",
			effect: "expansion",
			voteNeed: 0.5,
			impact: { type: "growth", price: 1.05, momentum: 0.01 },
		},
		{
			id: "cost_cutting",
			name: "Taglio Costi",
			...
			effect: "costs",
			voteNeed: 0.5,
			impact: { type: "margin", price: 1.03 },
		},
		{
			id: "ceo_change",
			name: "Cambio CEO",
			desc: "Sostituire il CEO con nuova leadership",
			effect: "ceo",
			voteNeed: 0.5,
			impact: { type: "volatility", price: 0.97, volUp: 1.3 },
		},
		{
			id: "stock_split",
			name: "Split 2:1",
			desc: "Raddoppiare le azioni, dimezzare il prezzo",
			effect: "split",
			voteNeed: 0.67,
			impact: { type: "split", factor: 2 },
		},
		{
			id: "special_dividend",
			name: "Dividendo Straordinario",
			desc: "Distribuire 2€ per azione una tantum",
			effect: "special_div",
			voteNeed: 0.5,
			impact: { type: "specialDiv", amount: 2 },
		},
	];
	...
	GameEngine.prototype._checkAssemblies = function () {
		...
		for (var i = 0; i < companies.length; i++) {
			...
			if (ownership > 0.01 && rng() < 0.05) {
				...
				var proposals = [];
				var pool = ASSEMBLY_PROPOSALS.slice().sort(function () {
					return rng() - 0.5;
				});
				for (var k = 0; k < proposalCount && k < pool.length; k++) {
					proposals.push({
						...
						playerVoted: false,
						playerVote: null,
						aiFor: Math.floor(c.sharesOutstanding * (0.1 + rng() * 0.25)),
						aiAgainst: Math.floor(c.sharesOutstanding * (0.05 + rng() * 0.2)),
				...
				this.notify(
					"info",
					"Assemblea " + c.ticker,
					"Hai diritto di voto. Possiedi " + (ownership * 100).toFixed(2) + "%",
				);
			}
		}
	};
	...
	GameEngine.prototype.voteAssembly = function (ticker, proposalIndex, vote) {
		var company = this.getCompany(ticker);
		if (!company || !company.assemblyScheduled)
			return { success: false, error: "Nessuna assemblea attiva" };
		var pos = this.state.portfolio.positions[company.id];
		...
		if (proposalIndex < 0 || proposalIndex >= proposals.length)
			return { success: false, error: "Proposta non valida" };
		var prop = proposals[proposalIndex];
		if (prop.playerVoted)
			return { success: false, error: "Hai gia votato questa proposta" };
		...
		this.notify(
			"success",
			"Voto Registrato",
			"Hai votato " + vote + " su " + prop.name + " con " + votes + " azioni",
		);
		this._checkAchievements();
	...
	GameEngine.prototype.resolveAssembly = function (ticker) {
		...
		var proposals = company.assemblyProposals || [];
		var totalVotes =
			company.assemblyVotes.for +
			company.assemblyVotes.against +
			company.assemblyVotes.abstain;
		var pos = this.state.portfolio.positions[company.id];
		for (var i = 0; i < proposals.length; i++) {
			...
			this.notify(
				"info",
				company.ticker + ": " + prop.name,
				msg +
					" (" +
					((forVotes / company.sharesOutstanding) * 100).toFixed(1) +
					"% a favore)",
			);
		}
	...
	GameEngine.prototype._applyAssemblyEffect = function (company, prop) {
		...
		var actorId =
			"ceo:" +
			(company.ceoName || company.name.split(" ")[0] + " CEO")
				.toLowerCase()
				.replace(/[^a-z0-9]/g, "_");
		if (impact.type === "div") {
			...
			company.sharesOutstanding = Math.max(
				1,
				Math.floor(company.sharesOutstanding * 0.95),
			);
			...
			this.notify(
				"success",
				"Split Azionario",
				company.ticker + " split " + factor + ":1 — azioni raddoppiate",
			);
			...
			if (position) {
				...
				this.notify(
					...
					"+" + formatMoney(payout) + " da " + company.ticker,
				);
			}
		}
		company.price = Math.max(0.1, roundCents(company.price));
		company.marketCap = company.price * company.sharesOutstanding;
		this.addNarrativeMemory(
			actorId,
			"Il board di " +
				company.name +
				' ha visto approvare la proposta "' +
				prop.name +
				'". Il mercato reagisce.',
			"positive",
			6,
	...
	GameEngine.prototype._computeMarketReturn = function () {
		...
		for (var i = 0; i < companies.length; i++) {
			...
			totalReturn +=
				((companies[i].price - companies[i].prevClose) /
					companies[i].prevClose) *
				companies[i].marketCap;
		}
	...
	GameEngine.prototype._generateEvents = function () {
		...
		if (eventType === "company") {
			...
			this.notify(
				"market",
				company.name + ": " + event.desc,
				event.impact > 0 ? "Rialzista" : "Ribassista",
			);
			...
			for (var i = 0; i < this.state.market.companies.length; i++) {
				if (this.state.market.companies[i].sector === sector) {
					...
					this.state.market.companies[i].price = Math.max(
						0.1,
						roundCents(this.state.market.companies[i].price),
					);
				}
			}
			...
			this.notify(
				"market",
				sector + ": " + event.desc,
				event.impact > 0 ? "Positivo" : "Negativo",
			);
		} else {
			// Macro event
			for (var j = 0; j < this.state.market.companies.length; j++) {
				this.state.market.companies[j].price *= 1 + event.impact * 0.3;
				this.state.market.companies[j].price = Math.max(
					0.1,
					roundCents(this.state.market.companies[j].price),
				);
			}
			this.state.market.sentiment = clamp(
				this.state.market.sentiment + event.impact * 100,
				0,
				100,
			);
			if (event.type === "rate_cut")
				this.state.market.macro.interestRate = Math.max(
					0,
					this.state.market.macro.interestRate - 0.25,
				);
			...
			this.notify(
				"market",
				"MACRO: " + event.desc,
				event.impact > 0 ? "Mercato rialzista" : "Mercato ribassista",
			);
		}
		return events;
	};
	...
	GameEngine.prototype._updatePrices = function () {
		...
		this.state.market.marketTrend = clamp(
			this.state.market.marketTrend +
				rngGauss(0, 0.001) -
				(this.state.market.marketTrend - 0.002) * 0.05,
			-0.005,
			0.01,
		);
		...
		var macroTrend =
			(macro.gdpGrowth - 1.0) * 0.001 - (macro.inflation - 2.5) * 0.0002;
		...
		for (var i = 0; i < companies.length; i++) {
			...
			c.price = Math.max(0.1, roundCents(c.price * (1 + change)));
		...
		var indexChange =
			prevTotalCap > 0 ? (totalCap - prevTotalCap) / prevTotalCap : 0;
	...
	GameEngine.prototype._processPendingOrders = function () {
		...
		for (var i = orders.length - 1; i >= 0; i--) {
			...
			if (order.type === "limit_buy" && company.price <= order.limitPrice)
				shouldExecute = true;
			if (order.type === "limit_short" && company.price >= order.limitPrice)
				shouldExecute = true;
			if (order.type === "limit_sell" && company.price >= order.limitPrice)
				shouldExecute = true;
			...
			if (shouldExecute) {
				...
				this.notify(
					"info",
					"Ordine Scaduto",
					"Ordine limite su " + order.ticker + " scaduto",
				);
			}
		}
	};
		...
		for (var i = 0; i < keys.length; i++) {
			...
			if (dividend > 0.01) {
				self.notify(
					"dividend",
					"Dividendo",
					company.ticker + ": +" + formatMoney(dividend),
				);
			}
		}
	};
	...
	GameEngine.prototype._processQuarterlyReports = function () {
		...
		for (var i = 0; i < companies.length; i++) {
			...
			if (c.earnings.netIncome > 0 && rng() < 0.3) {
				c.dividendYield = clamp(c.dividendYield + rngGauss(0, 0.005), 0, 0.1);
			}
		}
		this.notify(
			"info",
			"Report Trimestrali",
			"Nuovi report aziendali disponibili",
		);
	};
	...
	GameEngine.prototype._processIPOs = function () {
		...
		if (rng() < 0.15) {
			...
			this.notify(
				"ipo",
				"Nuova IPO: " + company.name,
				"Prezzo: " + formatMoney(price) + " - Settore: " + sector,
			);
		}
	};
	...
	GameEngine.prototype._updateSentiment = function () {
		...
		this.state.market.sentiment = clamp(
			this.state.market.sentiment + sentimentDrift,
			0,
			100,
		);
		// Ethics affects sentiment slightly
		if (this.state.player.ethics > 70)
			this.state.market.sentiment = clamp(
				this.state.market.sentiment + 0.5,
				0,
				100,
			);
	};
	...
	GameEngine.prototype._updateUnrealizedPnL = function () {
		...
		if (p.netWorth > p.stats.highestNetWorth)
			p.stats.highestNetWorth = p.netWorth;
	};
	...
	GameEngine.prototype.advanceWeek = function () {
		...
		for (var ai = 0; ai < assembCompanies.length; ai++) {
			...
			if (
				ac.assemblyScheduled &&
				ac.assemblyResolvedWeek &&
				this.state.player.week - ac.assemblyResolvedWeek >= 2
			) {
		...
		if (this._competitorEngine) {
			try {
				...
				this._competitorEngine.setPlayerData(
					posArr,
					this.state.player.cash,
					this.state.player.reputation.wallStreet || 50,
				...
				if (
					this._competitorEngine.competitors &&
					this._competitorEngine.competitors.length
				) {
					for (
						var cx = 0;
						cx < this._competitorEngine.competitors.length;
						cx++
					) {
				...
				if (
					...
					compResult &&
					compResult.trades &&
					compResult.trades.length
				) {
					for (var ct = 0; ct < compResult.trades.length; ct++) {
						var t = compResult.trades[ct];
						if (!t.competitor || (!t.competitor.name && !t.competitor.nickname))
							continue;
						var compName = t.competitor.name || t.competitor.nickname;
						this.addNarrativeMemory(
							"competitor:" + compName.toLowerCase().replace(/[^a-z0-9]/g, "_"),
							compName +
								" ha effettuato un'operazione " +
								t.action +
								" su " +
								(t.target || "il mercato") +
								".",
	...
	GameEngine.prototype.addNarrativeMemory = function (
		actorId,
		text,
		impact,
		importance,
		...
		var NarrativeEngine =
			typeof LLMNarrativeEngine !== "undefined" ? LLMNarrativeEngine : null;
		if (!NarrativeEngine && typeof require === "function") {
			try {
				NarrativeEngine = require("./llm-narrative-engine.js");
			} catch (e) {}
		}
		if (!NarrativeEngine || !NarrativeEngine.addMemory || !this.state)
			return false;
	...
	GameEngine.prototype.getPlayerInfo = function () {
		...
		return {
			...
			year: p.year,
			chapter: p.chapter,
			chapterName: CHAPTERS[p.chapter - 1]
				? CHAPTERS[p.chapter - 1].name
				: "Unknown",
	...
	GameEngine.prototype.getGameStats = function () {
		...
		return {
			...
			totalTrades: p.stats.totalTrades,
			winRate:
				p.stats.totalTrades > 0
					? p.stats.totalProfit / (p.stats.totalProfit + p.stats.totalLoss || 1)
					: 0,
			totalProfit: p.stats.totalProfit,
			totalLoss: p.stats.totalLoss,
