...
(function (global) {
  ...
  function buildCompetitorPrompt(competitors, gameState) {
    ...
    for (var j = 0; j < comps.length; j++) {
      prompt += comps[j].nickname + ' (strategia: ' + (comps[j].strategy || 'balanced') + ', capitale: 
    }
...
})(typeof window !== 'undefined' ? window : (typeof global !== 'undefined' ? global : this)); + (comps[j].capital || 0) + ', relazione: ' + (comps[j].relationship || 'neutral') + ', obiettivo: ' + (comps[j].goal || 'massimizzare il rendimento') + ', piano: ' + (comps[j].worldPlan || 'nessuno') + ', convinzione: ' + (comps[j].conviction || 50) + '), ';
    }
    prompt += 'Per ogni competitor genera una decisione. ';
    prompt += 'Rispondi con JSON array, uno per competitor: ';
    prompt += '{nickname, action, target, quantity, reason, aggression}. ';
